...
void getbmp16(char* inp, int uncompressedSize) {
	char outputname[16];
	const unsigned char bmpheader[66]={0x42,0x4D,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x42,0x00,0x00,0x00,0x28,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01,0x00,0x10,0x00,0x03,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xF8,0x00,0x00,0xE0,0x07,0x00,0x00,0x1F,0x00,0x00,0x00};
	unsigned long width=((unsigned int*)inp)[1], height=((unsigned int*)inp)[2], sizebm=width*height*2, sizebmp=sizebm+66;
	unsigned char *bmp=malloc(sizebmp); memcpy(bmp,bmpheader,66);
	memcpy(&bmp[2],&sizebmp,4); memcpy(&bmp[18],&width,4); memcpy(&bmp[22],&height,4); memcpy(&bmp[34],&sizebm,4);
	for (int y=0;y<height;y++) {
		unsigned short *from=(unsigned short*)(inp+16+(height-1-y)*2*width), *to=(unsigned short*)(bmp+66+y*2*width);
		for (int x=0;x<width;x++) to[x]=((from[x]&0x1F)<<11)|((from[x]&0x7C00)>>10)|((from[x]&0x03E0)<<1);
	}
